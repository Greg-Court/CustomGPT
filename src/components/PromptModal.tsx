...
const PromptModal = ({ userUid, open, handleClose }) => {
  ...
  useEffect(() => {
    if (userUid) {
      const fetchPrompts = async () => {
        ...
        setPrompts(promptsSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })));
      };
  ...
  const handleAddPrompt = async () => {
    const newPrompt = await createPrompt(userUid, newPromptContent, newPromptName);
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
          ...
          {prompts.map(prompt => (
            <ListItem key={prompt.id}>
              ...
              <ListItemSecondaryAction>
                <IconButton edge="end" onClick={() => handleDeletePrompt(prompt.id)}>
                  <Delete />
                </IconButton>
              </ListItemSecondaryAction>
            </ListItem>
        ...
        <TextField label="New Prompt Name" value={newPromptName} onChange={(e) => setNewPromptName(e.target.value)} fullWidth />
        <TextField label="New Prompt Content" value={newPromptContent} onChange={(e) => setNewPromptContent(e.target.value)} fullWidth multiline rows={4} />
      </DialogContent>
      <DialogActions>
